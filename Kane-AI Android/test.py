...
class FirstSampleTest(unittest.TestCase):
    ...
    def test_demo_site(self):
        ...
        try:    
            ...
            lambda_hooks(driver, "click on COLOR button")
            ui_action(driver = driver, operation_index = str(0))

            # click on GEOLOCATION button
            lambda_hooks(driver, "click on GEOLOCATION button")
            ui_action(driver = driver, operation_index = str(1))

            # type 'India' in the text field
            lambda_hooks(driver, "type 'India' in the text field")
            ui_action(driver = driver, operation_index = str(2))

            # click on 'FIND' button
            lambda_hooks(driver, "click on 'FIND' button")
            ...
            third_button_value = vision_query(driver = driver, operation_index = str(5))
            user_variables["third_button_value"] =  third_button_value
            print("third_button_value:", third_button_value)

            # assert {{third_button_value}} is equal to 'TEXT'
            lambda_hooks(driver, "assert {{third_button_value}} is equal to 'TEXT'")
            perform_assertion(driver, "6", str(third_button_value).lower().strip(), "==", """text""",{"assert {{third_button_value}} is equal to 'TEXT'"})
...
if __name__ == "__main__":
    ...
    fixed_caps = {'tms.tc_id': 'TC-1807', 'platformName': 'android', 'autoGrantPermissions': False, 'unicodeKeyboard': True}
